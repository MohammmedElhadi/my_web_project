...
@section('content')
                  ...
                  <div class="row gap-y gap-12" data-shuffle="list" >
                    ...
                    <div  id="{{ $event->id }}" class="row gap-y align-items-center" data-shuffle="item" data-groups="{{$event->category->nom}}@foreach($event->domaines as $domaine),{{ $domaine->nom }}@endforeach">
                      ...
                      <div class="col-md-6 ml-auto">
                          ...
                          @if( $event->etat == 'programe')
                            ...
                            @else
                              @if (Auth::check())
                                ...
                                <button id="inter.{{$event->id}}"
                                   @if (Auth::user()->isInteressetedIn($event) == false) 
                                   hidden
                                   @endif
                                    type="submit" class="btn btn-primary" onclick="iterrested('{{$event->id}}')">
                                  {{ __(" s'intéresser ") }}
                                </button>                                 
                             
                                <button id="desinter.{{$event->id}}" 
                                  @if (Auth::user()->isInteressetedIn($event)) 
                                    hidden
                                  @endif
                                  type="submit" class="btn btn-danger" onclick="desabonner('{{$event->id}}')" >
                                  {{ __(" annuler ") }}
                                </button> 
                        ...
                        <div class="nav-divider"></div>
                        <div class="col-md-auto">
                          <small class="text-lighter float-right nbr_tot">{{$event->nombre_participant}}</small>
                          <small  class="text-lighter float-right" >nombre des participant : </small>
      ...
      //  }, 5000);
          ...
          function handleDetail(data, nbr_participant , category , domaines){
            ...
            document.getElementById('nbr_rest_detail').innerHTML  = event.nombre_max_participant - nbr_participant;
            $('#modal-detail').modal('show');
          }

          function desabonner(event_id){
           var token = $("meta[name='csrf-token']").attr("content");
            $.ajax(
            {
                url: "desabonner/"+event_id ,
                type: 'GET',
                data:{
                  id : event_id
                },
                success: function (response) {
                  document.getElementById('desinter.'+event_id).setAttribute("hidden", true);
                  document.getElementById('inter.'+event_id).removeAttribute("hidden");
              }

            });
          }

          function iterrested(event_id){
           // console.log(id)
           var token = $("meta[name='csrf-token']").attr("content");
            $.ajax(
            {
                url: "abonner/"+event_id ,
                type: 'GET',
                data:{
                  id : event_id
                },
                success: function (response) {
                  document.getElementById('inter.'+event_id).setAttribute("hidden", true);
                  document.getElementById('desinter.'+event_id).removeAttribute("hidden");
              },
